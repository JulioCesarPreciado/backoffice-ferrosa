        ...
        <div class="w-full px-4">
            <div class="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-slate-100 border-0">
                ...
                <div class="flex-auto px-4 lg:px-10 py-10 pt-0">
                    ...
                    <form method="POST" action="{{ route('product-discount.update', $product_discount->id) }}" enctype="multipart/form-data"
                        autocomplete="off">
                        @csrf
                        @method('PUT')
                        <div
                            ...
                            <select name="product_id" id="product_id" class="producto-select w-full" required>
                                @foreach ($productos as $producto)
                                    @if($producto->id == $product_discount->product_id)
                                        <option title="{{ $producto->thumbnail }}" value="{{ $producto->id }}"
                                            {{ collect(old('product_id'))->contains($producto->id) ? 'selected' : '' }} selected>
                        ...
                        <div
                            ...
                            <input placeholder="{{ __('Product price') }}" value="{{ old('product_price', $product_discount->producto->price)}}" id="product_price"
                                class="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
                                disabled>
                        </div>
                        <div
                        ...
                        <div
                            ...
                            <label for="discount" class="block uppercase text-blueGray-600 text-xs font-bold mb-2">
                                {{ __('Discount') }}</label>
                            <input placeholder="{{ __('Discount') }}" id="discount" name="discount" value="{{ old('discount',$product_discount->discount) }}" readonly
                                class="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150">
                        </div>
                        <div class="w-full px-4 mt-4">
                            <input type="checkbox" id="status" @if ($product_discount->status == 'ACTIVO') checked @endif name="status" value="{{ old('status', $product_discount->status) }}" class="border-0 px-3 py-3 placeholder-slate-300 text-slate-600 bg-white rounded text-sm shadow focus:outline-none focus:ring ease-linear transition-all duration-150 hover:bg-slate-300">
...
@push('js')
    <script>
        ...
        flatpickr("#discount_start_date", {minDate: Date.now()});
        flatpickr("#discount_end_date", {});

        //setea el valor del product price
